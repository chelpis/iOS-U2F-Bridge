...
class U2FHid:
    ...
    def get_randomness(self, n_bytes):
        return os.urandom(n_bytes)

    def get_current_timestamp(self):
        return int(time.time() * 1000)

    def uhid_process_event_from_kernel(self, shouldStop):
        print("uhid_process_event_from_kernel start")
        # ev = uhid_parse_event_from_kernel(os.read(fd, 4380))
        while not shouldStop():
            if ([] != select.select([fd], [], [], 0.1)[0]):
                ev = self.uhid_parse_event_from_kernel(os.read(fd, 4380))
                break
            else:
                time.sleep(0.2)

        print("uhid_process_event_from_kernel end")
        if ev[0] == 2:
        ...
        if ev[0] == 6:
            ...
            self._process_request_packet(data[1:])


    def uhid_parse_event_from_kernel(self, event):
    ...
    def uhid_generate_create2_event(self):
        ev_type = 11
        name = b''
        phys = b''
        uniq = b''
    ...
    def uhid_generate_input2_event(self, input_report_data):
        ev_type = 12
    ...
    def _process_request_packet(self, octets):
        assert len(octets) == 64
        if octets[4] >> 7:
            self._process_request_initialization_packet(octets)
        else:
            self._process_request_continuation_packet(octets)



    
    def _process_request_initialization_packet(self, octets):
        global X

        initpkt = InitPacket(*struct.unpack(INIT_PACKET_FMT, octets))

        if initpkt.cmd not in (U2FHID_INIT, U2FHID_PING, U2FHID_MSG):
            self._send_response_message(initpkt.cid, U2FHID_ERROR, ERR_INVALID_CMD)
            return
        ...
        if current_time < X.deadline:
            ...
            self._process_request_message_MSG(initpkt.cid, initpkt.data[:initpkt.bcnt])
        elif initpkt.cmd == U2FHID_PING:
            self._process_request_message_PING(initpkt.cid, initpkt.data[:initpkt.bcnt])
    ...
    def _process_request_continuation_packet(self, octets):
        ...
        if len(X.data) >= X.bcnt:
            if X.cmd == U2FHID_MSG:
                self._process_request_message_MSG(X.cid, X.data[:X.bcnt])
            if X.cmd == U2FHID_PING:
                self._process_request_message_PING(X.cid, X.data[:X.bcnt])
            X = ALREADY_EXPIRED_STATE


    def generate_new_channel_id(self):
        while True:
            r = self.get_randomness(4)
            i = int.from_bytes(r, 'big')
            if i != 0 and i != 0xffffffff:
                return i


    def _process_request_message_INIT(self, nonce):
        #
        # response:
        #
        # cid = 0xffffffff
    ...
    def _process_request_message_MSG(self, cid, data):
        ...
        u2fraw.process_u2fraw_request(data, self.taskQueue)
        # _send_response_message(cid, U2FHID_MSG, resp)

    def send_response_message(self, payload):
        self._send_response_message(self.cid, U2FHID_MSG, payload)
    
    def _send_response_message(self, cid, cmd, payload):
        ...
        payload = payload[57:]
        self._sendout_response_packet(octets)
    ...
    def _sendout_response_packet(self, octets):
        assert len(octets) == 64
        self.uhid_generate_input2_event(octets)


    def setup_uhid(self):
        global fd
        fd = os.open('/dev/uhid', os.O_RDWR | os.O_NONBLOCK)
        self.uhid_generate_create2_event()
        # while True:
